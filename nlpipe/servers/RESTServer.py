...
def index():
    fsdir = app_restServer.client.result_dir
    mods = sorted(known_modules(), key=lambda mod: mod.name)
    mods = {mod: dict(app_restServer.client.statistics(mod.name)) for mod in mods}
    return render_template('index.html', **locals())


@app_restServer.route('/api/modules/<module>/', methods=['POST'])
@check_auth
def post_task(module):
    ...
    id = app_restServer.client.process(module, doc, id=id)
    resp = Response(id + "\n", status=202)
...
def task_status(module, id):
    ...
    """
    status = app_restServer.client.status(module, id)
...
def result(module, id):
    ...
    try:
        result = app_restServer.client.result(module, id, format=format)
...
def get_task(module):
    ...
    """
    id, doc = app_restServer.client.get_task(module)
    if doc is None:
...
def put_results(module, id):
    ...
    if request.content_type == ERROR_MIME:
        app_restServer.client.store_error(module, id, doc)
    else:
        app_restServer.client.store_result(module, id, doc)
    return '', 204
...
def bulk_status(module):
    ...
    statuses = {id: app_restServer.client.status(module, str(id)) for id in ids}
...
def bulk_result(module):
    ...
    try:
        ...
        return "Error: Please provive bulk IDs as a json list\nd ", 400
    format = request.args.get('format', None)
    results = app_restServer.client.bulk_result(module, ids, format=format)
...
def bulk_process(module):
    ...
    ids = app_restServer.client.bulk_process(module, docs, ids=ids, reset_error=reset_error, reset_pending=reset_pending)
    return jsonify(ids)
